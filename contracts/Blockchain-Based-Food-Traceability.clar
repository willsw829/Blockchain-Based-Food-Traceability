...
(define-public (add-producer-certification 
  ...
  (begin
    (asserts! (is-eq tx-sender contract-owner) err-owner-only)
    (ok (map-set producer-certifications
      { producer: producer }
      {
...
(define-public (update-product-stage
  ...
  (let
    (
      ...
      (stage-count-data (unwrap! (map-get? product-stage-count { product-id: product-id }) err-not-found))
    ...
    (map-set product-history
      { product-id: product-id, stage-id: new-count }
      {
        stage: new-stage,
        location: location,
        timestamp: stacks-block-height,
...
(define-read-only (verify-organic-claim (product-id uint))
  (match (map-get? products { product-id: product-id })
    product (let
      (
        (producer-cert (map-get? producer-certifications { producer: (get producer product) }))
      )
      (and 
